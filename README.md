...
```ts
// checkmate position - black marshal surrounded with no escape
const gungi = new Gungi(
...
```ts
// stalemate position - black marshal cornered, not in check, but all moves lead to check
...
```ts
// only two non-adjacent marshals remain
