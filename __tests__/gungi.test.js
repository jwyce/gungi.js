...
describe('Check', () => {
    ...
    test('pinned piece', () => {
        expect(gungi.move({src: {type: gungi.ARCHER, color: gungi.WHITE}, dst: '1-1', type: gungi.PLACE})).toBeNull();
    })
    ...
    describe('Game Phase', () => {
        beforeEach(() => {
            ...
            gungi.move({src: {type: gungi.ARCHER, color: gungi.WHITE}, dst: '1-5', type: gungi.PLACE})
            gungi.move({src: {type: gungi.MUSKETEER, color: gungi.BLACK}, dst: '8-5', type: gungi.PLACE})
            gungi.move({src: {type: gungi.FORTRESS, color: gungi.WHITE}, dst: '3-5', type: gungi.PLACE})
            gungi.move({src: null, dst: null, type: gungi.READY})
            gungi.move({src: null, dst: null, type: gungi.READY})
        });

        test('pinned piece', () => {
            expect(gungi.move({src: '3-5', dst: '3-4', type: gungi.PLACE})).toBeNull();
        })
    })
...
describe('Move Generation', () => {
    ...
    beforeEach(() => {
        ...
        gungi.move({src: {type: gungi.MARSHALL, color: gungi.BLACK}, dst: '7-7', type: gungi.PLACE})
        gungi.move({src: {type: gungi.MARSHALL, color: gungi.WHITE}, dst: '3-6', type: gungi.PLACE})
        gungi.move({src: {type: gungi.MUSKETEER, color: gungi.BLACK}, dst: '7-8', type: gungi.PLACE})
        gungi.move({src: {type: gungi.PAWN, color: gungi.WHITE}, dst: '2-8', type: gungi.PLACE})
        gungi.move({src: null, dst: null, type: gungi.READY})
    });

    test('marshall cannot be stacked on', () => {
        expect(gungi.move({src: {type: gungi.SPY, color: gungi.WHITE}, dst: '3-6', type: gungi.PLACE})).toBeNull();
    })

    test('fotress cannot stack on pieces', () => {
        expect(gungi.move({src: {type: gungi.FORTRESS, color: gungi.WHITE}, dst: '2-8', type: gungi.PLACE})).toBeNull();
    })

    test('pawn cannot be placed in same file as pawn of the same color', () => {
        expect(gungi.move({src: {type: gungi.PAWN, color: gungi.WHITE}, dst: '3-8', type: gungi.PLACE})).toBeNull();
    })

    test('pieces cannot be placed outside your first 3 ranks in draft phase', () => {
        expect(gungi.move({src: {type: gungi.SPY, color: gungi.WHITE}, dst: '4-1', type: gungi.PLACE})).toBeNull();
    })
})
